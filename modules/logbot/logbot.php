...
class Logbot extends Module {
    ...
    public function PRIVMSG(IRCMessage $Message) {
        ...
        if ($channel == 'PM') {
            ...
            if (preg_match('/^!logbot help$/', $msg)) {
                $this->_sendHelpMessage($Author,$Channel);
            }
     ...
     * @param Channel $Channel
     */
    private function _sendHelpMessage(Author $Author, Channel $Channel) {
        global $bot;
        $messages[] = Tokeniser::tokenise('logbot', 'PRIVACY_WARNING_1', ['nick' => $Author->name, 'channel' => $Channel->name, 'botname' => Config::get('irc_nick')]);
        $messages[] = Tokeniser::tokenise('logbot', 'PRIVACY_WARNING_2', ['channel' => $Channel->name]);
        $messages[] = Tokeniser::tokenise('logbot', 'PRIVACY_WARNING_3');
        $messages[] = Tokeniser::tokenise('logbot', 'PRIVACY_WARNING_4');
        ...
        if($Author->name == Config::get('owner')) {
